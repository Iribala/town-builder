...
async def _handle_proxy_request(
    request: Request, method: str, path: str = "", data: dict | None = None
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=504, detail="Request to upstream service timed out"
        )
    except httpx.ConnectError:
        logger.error(f"Connection error proxying request")
        raise HTTPException(
            status_code=503, detail="Could not connect to upstream service"
        )
...
@router.get("")
async def proxy_towns_get_root(
